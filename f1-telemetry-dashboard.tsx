...
const sty = {
  bg: {
    ...
    background: "linear-gradient(180deg, #08080f 0%, #0c0c18 100%)",
    color: "#e8e8ec",
    minHeight: "100vh",
    padding: 0,
  },
...
function mergeDistance(cd, loc) {
  ...
  for (let i = 0; i < loc.length; i++) {
    ...
    locDist.push({ t: new Date(loc[i].date).getTime(), distance: cum });
  }
  return cd.map(c => {
    const t = new Date(c.date).getTime();
    if (!locDist.length) return { ...c, distance: 0 };
    let best = 0, bestDiff = Math.abs(locDist[0].t - t);
    for (let j = 1; j < locDist.length; j++) {
      const diff = Math.abs(locDist[j].t - t);
...
function drawGrid(ctx, W, H, maxDist, plotTop, plotH) {
  ...
  for (let d = 0; d <= maxDist; d += step) {
    ...
    ctx.fillText(d >= 1000 ? (d / 1000).toFixed(0) + "k" : String(d), x, plotTop + plotH + 18);
  }
...
function Chart({ traces, syncRef }) {
  ...
  useEffect(() => {
    ...
    const maxDist = Math.max(...traces.flatMap(t => t.data.map(d => d.distance ?? 0)), 1);
    const maxSpd = Math.max(...traces.flatMap(t => t.data.map(d => d.speed)), 1);
      ...
      panels.forEach(p => {
        if (p.isDrs) {
          ...
          for (let i = 0; i < n; i++) {
            ...
            const isOpen = [10, 12, 14].includes(drs);
            const isEligible = drs === 8;
  ...
  const drawOverlay = useCallback((hoverDist) => {
    ...
    // Find nearest data point per trace
    const findNearest = (data, targetDist) => {
      let best = 0, bestDiff = Infinity;
      for (let i = 0; i < data.length; i++) {
        const diff = Math.abs((data[i].distance ?? 0) - targetDist);
    ...
    if (points.length) {
      const header = "DIST " + Math.round(hoverDist).toLocaleString() + "m";
...
export default function App() {
  ...
  const [loading, setLoading] = useState("Loading 2026 races...");
  const [error, setError] = useState("");
  const syncRef = useRef({});

  useEffect(() => {
    api("/meetings?year=2026")
      .then(d => { setMeetings(d); setLoading(""); })
      .catch(e => { setError(e.message); setLoading(""); });
  }, []);

  const onMeeting = useCallback((v) => {
    setMk(v); setSk(""); setDn(""); setSessions([]); setDrivers([]); setLaps([]); setCarData([]); setComparisons([]);
    if (!v) return;
    setLoading("Loading sessions...");
    api("/sessions?meeting_key=" + v)
      .then(d => { setSessions(d); setLoading(""); })
      .catch(e => { setError(e.message); setLoading(""); });
  }, []);

  const onSession = useCallback((v) => {
    setSk(v); setDn(""); setDrivers([]); setLaps([]); setCarData([]); setComparisons([]);
    if (!v) return;
    setLoading("Loading drivers...");
    Promise.all([
      api("/drivers?session_key=" + v),
      api("/weather?session_key=" + v).catch(() => []),
      api("/race_control?session_key=" + v).catch(() => []),
      api("/session_result?session_key=" + v).catch(() => []),
    ]).then(([d, w, r, sr]) => {
    ...
    <div style={sty.bg}>
      ...
      <div style={{ padding: "16px 24px" }}>
        ...
        <div style={{
          ...
          {sessions.length > 0 && (
            <div style={{ flex: 0.6, padding: "14px 18px", borderRight: "1px solid rgba(255,255,255,0.04)" }}>
              <div style={{
                fontSize: 9,
                fontWeight: 600,
                color: "#5a5a6e",
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  ...
                  <div>
                    <div style={{
                      ...
                      <span style={{ color: "rgba(255,255,255,0.4)", fontWeight: 500, marginRight: 6 }}>#{drv.driver_number}</span>
                      {drv.full_name}
                    </div>
                    <div style={{
                              ...
                              {l.date_start && l.lap_duration ? (
                                <span style={{ display: "inline-flex", gap: 4 }}>
                                  ...
                                  <button
                                    ...
                                    disabled={comparisons.some(c => c.id === dn + "-" + l.lap_number)}
                                    style={{
                                      background: comparisons.some(c => c.id === dn + "-" + l.lap_number) ? "rgba(225,6,0,0.15)" : "transparent",
                                      color: comparisons.some(c => c.id === dn + "-" + l.lap_number) ? "#e10600" : "#5a5a6e",
                                      border: comparisons.some(c => c.id === dn + "-" + l.lap_number) ? "1px solid rgba(225,6,0,0.3)" : "1px solid rgba(255,255,255,0.08)",
                                      borderRadius: 6,
                                      padding: "4px 8px",
                                      cursor: "pointer",
                                      fontSize: 11,
                                  ...
                                  >
                                    {comparisons.some(c => c.id === dn + "-" + l.lap_number) ? "\u2713" : "+"}
                                  </button>
                                </span>
                              ) : null}
                            </td>
                    ...
                    {weather.slice(-30).map((w, i) => (
                      <div key={i} style={{
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}>
                          ...
                          <span style={{ fontWeight: 600, fontFamily: "'JetBrains Mono','SF Mono',monospace" }}>{w.wind_speed} m/s</span>
                        </div>
                      </div>
                    ))}
                  </div>
